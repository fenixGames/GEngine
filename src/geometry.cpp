...
void
Arc2D::print() {
    int idx, limit = POINT_PREC * this->angle;
    GLfloat ang_step = 1 / POINT_PREC, ang;
    ...
    /* If the angle of the sector is negative, the step should also be negative. */
    if (this->angle < 0)
        ang_step *= -1;
 ...
 */
Circunference2D::Circunference2D(Point2D c, GLint radius) : Arc2D(c, Point2D(c.x + radius, c.y))
{}
